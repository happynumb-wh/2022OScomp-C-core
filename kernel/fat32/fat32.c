...
int fat32_init()
{
    ...
    if(kmemcmp((char*)(b + BS_FileSysType), "FAT32", 5)){
    ...
    kmemset(cwd.name, 0, 256);
    kmemset(root.name, 0, 256);
    kstrcat(cwd_path,"/");
    kstrcat(root.name,"/");
...
int fat32_openat(fd_num_t fd, const uchar *path, uint32 flags, uint32 mode)
{
    ...
    uchar copy_path[kstrlen(path) + 2];
    /* like / */
    /* open a dir */
    kstrcpy(copy_path, pre_path(path, base_clus));
    /* clear '/' */
    ...
    if (copy_path[0] == NULL) {
        ...
        kfree(buff);
        // init_share_fd(new_fd);
        // printk("pid:%d, fd:%d, clus: %d\n", current_running->pid, new, new_fd->first_clus_num);
        return new;
    }
    while (1) {
...
int64 fat32_read(fd_num_t fd, uchar *buf, size_t count)
{
    ...
    while (ncount < real_count) {
        ...
        /* memcpy */
        kmemcpy(buf_point, buffer + pos_offset_in_buf, readsize); 
        buf_point += readsize;
        ncount += readsize;
        nfd->pos += readsize;
...
int64 fat32_write(fd_num_t fd, uchar *buf, uint64_t count)
{
    ...
    while (ncount < count) {
        ...
        disk_read(buffer, nfd->cur_sec, READ_MAX_SEC);
        /* write */
        kmemcpy(buffer + pos_offset_in_buf, buf_point, writesize);
...
int64 fat32_lseek(fd_num_t fd, size_t off, uint32_t whence)
{
    ...
    if (!kstrcmp(nfd->name, "/etc/passwd") || 
        !kstrcmp(nfd->name, "/etc/localtime") ||
        !kstrcmp(nfd->name, "/etc/adjtime") ||
        !kstrcmp(nfd->name, "/etc/group") ||
        !kstrcmp(nfd->name, "/proc/mounts") ||
        !kstrcmp(nfd->name, "/proc/meminfo") ||
        !kstrcmp(nfd->name, "/ls")) 
    {
        return nfd->pos;
    }    
...
long fat32_getcwd(char *buf, size_t size){
    if(size<2){
        ...
        kmemcpy(buf, cwd_path, kstrlen(cwd_path)+1);
    }
    return buf;    
}
...
int fat32_chdir(char *dest_path)
{
    ...
    int len = kstrlen(dest_path);
    kstrcpy(temp, dest_path);
    dentry_t entry;
    int count = 0;
    for (int i = 0; i < len; i++)
    {
    ...
    path[count] = '\0';
    len = count;
    // printk("path: %s, len: %d, %d\n", path, len, kstrlen(path));
    // while(1);
    if(!kstrcmp("/", path))
    {
        kmemset(cwd_path, 0, FAT32_MAX_PATH);
        cwd_path[0] = '/';
        cwd_path[1] = '\0';
    ...
    if(dentry == NULL){
        ...
        kstrcpy(cwd.name, f_dir.name);
        if(dest_path[0] == '/'){
            kmemset(cwd_path, 0, FAT32_MAX_PATH);
            kmemcpy(cwd_path, dest_path, kstrlen(dest_path));
        }else{
            kstrcat(cwd_path, dest_path);
        }
        kfree(buff);
...
{  
    ...
    int len = kstrlen(path_const);
    char path[len + 1];
    kstrcpy(path, path_const);
    ...
    if(dir.name[0] == 0xff)
    {
        ...
        if(search(dir.name, dir.first_clus, buf, FILE_DIR, NULL) == 0){
            kmemset(buf, 0, sizeof(PAGE_SIZE));
...
int fat32_getdents(uint32_t fd, char *outbuf, uint32_t len)
{
    ...
    {
        kmemset(filename, 0, FAT32_MAX_FILENAME);
...
int fat32_unlink(int dirfd, const char* path, uint32_t flags)
{
    ...
    pcb_t * current_running = get_current_running();
    if (current_running->pid != 1 && !kstrcmp(((char **)((visit_command_line())))[1], "lat_fs"))
        if(temp_file(path)) {
            int i = 10000;
            while (i--); 
            return 0;
...
int32_t fat32_fcntl(fd_num_t fd, int32_t cmd, int32_t arg){
    ...
    else if (cmd == F_SETLK) kmemcpy((char *)&current_running->pfd[fd_index].flock,(char *)arg,sizeof(flock_t));
    else if (cmd == F_GETLK) kmemcpy((char *)arg,(char *)&current_running->pfd[fd_index].flock,sizeof(flock_t));
...
int fat32_fstatat(fd_num_t dirfd, const char *pathname, struct stat *statbuf, int32 flags){
    ...
    for (i=0; i<MAX_FILE_NUM; i++){
        if (current_running->pfd[i].used && !kstrcmp(current_running->pfd[i].name,file_name)) {
            exist = 1;
            break;
        }
    }
...
int fat32_renameat2(fd_num_t olddirfd, const char *oldpath_const, fd_num_t newdirfd, 
    ...
    char old_path_copy[kstrlen(oldpath_const) + 1];
    inode_t old_dir = {0};
    kstrcpy(old_path_copy, oldpath_const);
    if(oldpath_const[0] == '/')
    {
    ...
    char new_path_copy[kstrlen(newpath_const) + 1];
    inode_t new_dir = {0};
    kstrcpy(new_path_copy, newpath_const);
    if(newpath_const[0] == '/')
    {
