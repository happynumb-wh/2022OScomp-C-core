                        ...
                        struct sigaction *oldact, size_t sigsetsize)
{
    ...
    if (oldact) kmemcpy(oldact, this_sigaction, sizeof(sigaction_t));
    // handle shared handler process / thread in a recursive way
    if (act) kmemcpy(this_sigaction, act, sizeof(sigaction_t));
    return 0;
}
    ...
    for (uint8_t i = 0; i < NUM_SIG; i++)
    {
        ...
        if (ISSET_SIG(i + 1, current_running)){
            ...
            if (current_running->sigactions[i].handler == SIG_IGN)
                ...
                /* set a trampoline */
                kmemcpy(&signal_context_buf, current_running->save_context, sizeof(regs_context_t));
                kmemcpy(&signal_switch_buf, current_running->switch_context, sizeof(switchto_context_t));
                /* set ucontext */
                ...
                // signal_context_buf.regs[12] = (reg_t)(SIGNAL_HANDLER_ADDR - sizeof(ucontext_t));               /* a2 is uncontext */
                signal_context_buf.regs[1]  = (reg_t)(USER_STACK_ADDR - SIZE_RESTORE);                         /* should do sigreturn when handler return */
                ...
                uint64_t offset = (sizeof(regs_context_t) + sizeof(switchto_context_t));
                // new kernel_stack 
                signal_switch_buf.regs[1] = pcb->kernel_sp - offset;
                // another one sp
                current_running->kernel_sp -= offset;
                current_running->save_context = (uint64_t)(current_running->save_context) - offset;
                current_running->switch_context = (uint64_t)(current_running->switch_context) - offset;
...
uintptr_t set_ucontext()
{
    ...
    if (!need_alloc_page)
        alloc_page_helper(trap_sp_base, current_running->pgdir, MAP_USER);
    // remember fence
    if (!need_alloc_page)
        local_flush_tlb_all();
...
void init_sig_table(){
    for (int i = 0; i < MAX_SIGNAL_NUM; i++)
    {
        sig_table[i].num = 0;
        sig_table[i].used = 0;
        kmemset(sig_table[i].sigactions,0,NUM_SIG * sizeof(sigaction_t));
    }
}
...
void free_sig_table(sigaction_t* sig_in){
    ...
    if(st->num == 0){
        st->used = 0;
        kmemset(sig_in,0,NUM_SIG * sizeof(sigaction_t));
    }
}