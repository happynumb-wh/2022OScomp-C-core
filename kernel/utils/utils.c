...
{    
    ...
    // printk("n1:%s, n2:%s\n", n1, n2);
    return kmemcmp(n1, n2, kstrlen(n2));
}
...
inode_t find_dir(inode_t cur_cwd, char *name)
{
    ...
    int pos, count = 0;
    kmemset(input_name, 0, FAT32_MAX_FILENAME);
    kmemcpy(input_name, name, FAT32_MAX_FILENAME);
    name = input_name;
    if(name[0] == '.' && name[1] =='/'){
        name = name + 2;
    }
    ...
    if(count == 0){
        kmemset(cur_cwd.name, 0, FAT32_MAX_PATH);
        kmemcpy(cur_cwd.name, name, kstrlen(name));
        return cur_cwd;
    }else if(pos == len){
        kmemset(cur_cwd.name, 0, FAT32_MAX_PATH);
        kmemcpy(cur_cwd.name, name, kstrlen(name) - 1);
        return cur_cwd;
    }else{
        kmemset(cur_name, 0, FAT32_MAX_FILENAME);
        kmemset(new_name, 0, FAT32_MAX_FILENAME);
        kmemcpy(cur_name, name, pos);
        kmemcpy(new_name, &name[pos+1], len-pos-1);
...
dentry_t* create_new(char *name, uint32_t cur_clus, uchar *buf, type_t type, struct dir_pos *pos_t)
{
    ...
    len_filename = kstrlen(name);
    filename = name;
    extname = name + pos + 1;
    len_extname  = kstrlen(name + pos + 1);
    if(isfile)
        name[pos] = '.';
    // printk("name:%s\n", name);
    dentry_t *entry;
    ...
    //short_entry
    dentry_t new_dentry;
    kmemset(&new_dentry, 0, sizeof(new_dentry));
    ...
    for (int i = 0; i < dentry_num - 1; i++)
    {
        kmemcpy(entry, &long_entry[dentry_num - 2 - i], sizeof(dentry_t));
    ...
    kmemcpy(entry, &new_dentry, sizeof(new_dentry));
    disk_write(buf, sec, READ_MAX_SEC);
    // printk("entry:%lx\n", entry);
                         ...
                         dentry_t *p, uint32_t flags, dir_pos_t * pos)
{
    ...
    kstrcpy(pcb_under->pfd[i].name, name);
    /* copy dir to find the dir */
    kmemcpy(&pcb_under->pfd[i].dir_pos, pos, sizeof(dir_pos_t));
    return 0;
}
/* WH add */
/**
...
{
    if (!kstrcmp("/dev/null",path) || !kstrcmp("/dev/null/invalid",path)){
        new_fd->dev = DEV_NULL;
        new_fd->flags |= S_IFCHR;
        new_fd->nlink = 1;
        return 1;
    }
    else if (!kstrcmp("/dev/zero",path)) {
        new_fd->dev = DEV_ZERO;
        new_fd->flags |= S_IFCHR;
        new_fd->nlink = 1;
        return 1;
    }
    else if (!kstrcmp("/etc/passwd",path) ||
             !kstrcmp("/etc/localtime", path) ||
             !kstrcmp("/etc/adjtime", path) ||
             !kstrcmp("/etc/group", path) ||
             !kstrcmp("/proc/mounts", path) ||
             !kstrcmp("/proc/meminfo", path) ||
             !kstrcmp("/ls", path)) {
        ...
        new_fd->pos = 0;
        new_fd->nlink = 1;
        if (!kstrcmp("/etc/passwd",path)) new_fd->flags |= S_IFCHR;
        else new_fd->flags |= S_IFREG;
        return 1;     
    }else if (!kstrcmp("/dev/tty",path)) {
        kstrcpy(new_fd->name, path);
...
int read_special_path(fd_t * nfd, uchar *buf, size_t count)
{
        ...
        !kstrcmp(nfd->name, "/ls")) 
    {
        ...
        kmemset(buf,0,count);
        kmemcpy(buf,  elf_files[file_id].file_content + nfd->pos, copy_len);
        nfd->pos += copy_len;
        if(nfd->pos >= nfd->length){
            buf[copy_len] = (-1);//EOF
        }
...
int write_special_path(fd_t * nfd, uchar *buf, size_t count)
{
        ...
        !kstrcmp(nfd->name, "/ls")) 
    {
        ...
        kmemcpy(elf_files[file_id].file_content + nfd->pos, buf, write_len);
        nfd->pos += write_len;
        if(nfd->pos >= nfd->length){
            buf[write_len] = (-1);//EOF
        }
