...
static int handle_select(fd_num_t fd, int mode, uint64_t timeout_ticks)
{
    ...
    if (status_ctx)
    {
        ctx_pipe_t * fdp = &current_running->ctx_pipe_array[fd];
        ...
        if (mode == SELECT_READ){
            ...
            else if (fdp->pipe_type){            
                return wait_ring_buffer_read(&(pipe[fdp->fd_num].rbuf), final_ticks);;
            }
            else
                assert(0);
        }
            ...
            if (fdp->dev == DEV_STDOUT){
                ...
                return wait_ring_buffer_write(&(pipe[fdp->fd_num].rbuf), final_ticks);
            }
            else
