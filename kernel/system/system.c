...
int do_getrlimit(int resource,struct rlimit * rlim){
    ...
    kmemcpy(&mylimit,&current_running->fd_limit,sizeof(struct rlimit));
    kmemcpy(rlim, &mylimit, sizeof(struct rlimit));
    return 0;
}

int do_setrlimit(int resource,const struct rlimit * rlim){
    pcb_t *current_running = get_current_running();
    kmemcpy(&mylimit, rlim, sizeof(struct rlimit));
    kmemcpy(&current_running->fd_limit,&mylimit,sizeof(struct rlimit));
    return 0;
}
void do_syslog(){
    return;
