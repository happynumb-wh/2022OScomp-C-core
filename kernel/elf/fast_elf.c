...
int find_name_elf(const char * filename)
{
    for (int i = 0; i < MAX_TEST_NUM; i++)
    {
        if (pre_elf[i].used && !kstrcmp(filename, pre_elf[i].name))
            return i;
    }
    return -1;
}
...
int do_pre_load(const char * filename, int how)
{
    ...
    kstrcpy(pre_elf[free_index].name, filename);
...
// fast load elf
uintptr_t fast_load_elf(const char * filename, uintptr_t pgdir, uint64_t *file_length, void *initpcb)
{
...
for_page_remain: ;
    ...
    kmemcpy(&underinitpcb->elf, &exe_load->elf, sizeof(ELF_info_t));
    underinitpcb->edata = exe_load->elf.edata;

    return underinitpcb->elf.entry;
 ...
 * 
 */
uintptr_t fast_load_elf_execve(const char * filename, void *initpcb)
{
    // change to pcb_t
    pcb_t *underinitpcb = (pcb_t *)initpcb;
