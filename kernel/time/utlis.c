...
uint64_t timeval_to_ticks(timeval_t *tms){
    ...
    for (uint8_t i = 0; i < MICRO; i++){
        uticks += TIMEBASE * (usec % 10);
        uticks /= 10;
        usec /= 10;
    }
    return uticks + tms->tv_sec * TIMEBASE;  
}
...
{
    // time /= 10;
    tms->tv_sec = time / TIMEBASE; /* compute second */

    uint64_t left = time % TIMEBASE; /* compute micro seconds */
    tms->tv_usec = 0;
    for (uint i = 0; i < MICRO; ++i)
    {
        tms->tv_usec = 10*tms->tv_usec + left * 10 / TIMEBASE;
        left = (left * 10) % TIMEBASE;
    }    
}
