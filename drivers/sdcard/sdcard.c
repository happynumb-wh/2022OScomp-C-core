...
#else
	kmemcpy(data_buff, QEMU_DISK_OFFSET + (sector << 9), count*512);
	return 0;
#endif
}
...
uint8_t sd_read_sector_bio(struct buf *a, uint32_t count)
{
	...
	while (count-->0) {
		...
		kmemcpy(a->data,QEMU_DISK_OFFSET + (a->sectorno << 9),512);
		a++;
	}
	return 0;
#endif
...
uint8_t sd_write_sector(uint8_t *data_buff, uint32_t sector, uint32_t count)
{
	...
	return 0;
#else
	kmemcpy(QEMU_DISK_OFFSET + (sector << 9), data_buff, count*512);
	return 0;
#endif
}
...
uint8_t sd_write_sector_bio(struct buf *a, uint32_t count)
{
	...
	while (count-->0) {
		...
		kmemcpy(QEMU_DISK_OFFSET + (a->sectorno << 9), a->data, 512);
		a++;
	}
	return 0;
#endif
