...
name_table map_name[7] = { 
    {.sname = "etclocaltime", .dname = "/etc/localtime"},
    {.sname = "etcpasswd", .dname = "/etc/passwd"},
    {.sname = "etcadjtime", .dname = "/etc/adjtime"},
    {.sname = "etcgroup", .dname = "/etc/group"},
    {.sname = "procmounts", .dname = "/proc/mounts"},
    {.sname = "procmeminfo", .dname = "/proc/meminfo"},
    {.sname = "binls", .dname = "/ls"}
...
char* simplify(Records* self, char* varname)
{
    ...
    simplify_filter(ret, varname, "_test_");
    simplify_filter(ret, ret, "_");
    simplify_filter(ret, ret, "elf");
    simplify_filter(ret, ret, "disk");
    // simplify_filter(ret, ret, "exe");
    // simplify_filter(ret, ret, "disk");
    return handle_name(ret);
}
